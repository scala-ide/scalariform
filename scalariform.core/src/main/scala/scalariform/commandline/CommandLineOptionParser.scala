...
class CommandLineOptionParser extends JavaTokenParsers with RegexParsers {
  ...
  lazy val option: Parser[CommandLineArgument] = phrase(help) | phrase(test) | phrase(inPlace) | phrase(verbose) | phrase(fileList) | phrase(toggle) | phrase(preferenceOption) | phrase(badOption)
